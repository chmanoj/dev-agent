...
class TestErrorRecoveryScenarios:
    ...
    def test_file_permission_recovery(self):
        ...
        error = SystemError(
            ...
            context=ErrorContext(
                operation="state_save", file_path=".dev_agent/state.json"
            ),
    ...
    def test_error_recovery_execution_failure(self):
        ...
        with patch.object(
            self.error_handler, "_execute_recovery_action", return_value=False
        ):
            error = SystemError(
                message="Test system error", severity=ErrorSeverity.MEDIUM
            )
    ...
    def test_error_recovery_strategy_selection(self):
        ...
        assert hasattr(recovery, "use_partial_index") or hasattr(
        ...
        assert hasattr(recovery, "check_permissions") or hasattr(
            recovery, "create_directories"
        )
        ...
        assert hasattr(recovery, "reduce_memory_usage") or hasattr(
    ...
    def test_error_severity_handling(self):
        ...
        critical_error = SystemError(
            message="Critical system failure", severity=ErrorSeverity.CRITICAL
        )
        ...
        with patch.object(self.error_handler.logger, "error") as mock_error:
            with patch.object(self.error_handler.logger, "warning") as mock_warning:
                with patch.object(
                    ...
                    error1 = SystemError(
                        "High severity error", severity=ErrorSeverity.HIGH
                    )
                    error2 = PerformanceError(
                        "Medium severity error", severity=ErrorSeverity.MEDIUM
                    )
                    error3 = SystemError(
...
class TestErrorHandlerIntegration:
    ...
    def test_error_handler_with_mock_indexing_engine(self):
        ...
        mock_indexing_engine.build_index.side_effect = IndexingError(
            "Mock indexing failure", failed_files=["test.py"], partial_success=True
        )
