...
from ..models.analysis import (
    SpecificationAnalysis, DesignAnalysis, ArchitectureInfo, CodePatterns,
    CodeExample, CodeContext, ArchitecturePattern, CodePattern,
    RequirementEvidence, ComponentAnalysis, ContextualCode
)
...
class CodebaseAnalyzer(ICodebaseAnalyzer):
    ...
    def __init__(self, indexing_engine: IIndexingEngine):
        ...
        self.ast_index: Optional[ASTIndex] = None
        self._load_index()
    ...
    def _load_index(self) -> None:
        ...
        try:
            ...
            if symbol_map:
                if hasattr(self.indexing_engine, 'ast_index') and self.indexing_engine.ast_index:
                    self.ast_index = self.indexing_engine.ast_index
                else:
                    print("Warning: No AST index available. Some analysis features may be limited.")
            else:
    ...
    def _infer_project_purpose(self) -> str:
        ...
        common_words = [word for word, count in domain_keywords.most_common(10) if count > 1]
    ...
    def _extract_main_features(self) -> List[str]:
        ...
        action_features = set()
        for func_def in self.ast_index.functions.values():
            if func_def.name.startswith(('create', 'update', 'delete', 'get', 'set', 'process', 'handle')):
                words = re.findall(r'[A-Z][a-z]+|[a-z]+', func_def.name)
                if len(words) > 1:
                    feature = ' '.join(words[1:]).lower()
    ...
    def _identify_user_roles(self) -> List[str]:
        ...
        role_keywords = ['user', 'admin', 'client', 'customer', 'manager', 'operator', 'developer']
    ...
    def _identify_functional_areas(self) -> List[str]:
        ...
        for file_path in self.ast_index.file_metadata.keys():
            ...
            for part in path_parts:
                if part not in ['.', '..', '__pycache__', '.git'] and not part.startswith('.'):
    ...
    def _gather_requirement_evidence(self) -> List[RequirementEvidence]:
        ...
        crud_functions = []
        for func_def in self.ast_index.functions.values():
            if any(func_def.name.lower().startswith(op) for op in ['create', 'read', 'update', 'delete', 'get', 'set']):
        ...
        if crud_functions:
            evidence.append(RequirementEvidence(
                ...
                supporting_files=list(set(func.file_path for func in self.ast_index.functions.values() 
        ...
        auth_indicators = []
        for func_def in self.ast_index.functions.values():
            if any(keyword in func_def.name.lower() for keyword in ['auth', 'login', 'logout', 'permission', 'access']):
    ...
    def _analyze_external_dependencies(self) -> List[str]:
        ...
        for import_stmt in self.ast_index.imports:
            ...
            if module not in ['os', 'sys', 'json', 'time', 'datetime', 'collections', 'typing', 're']:
    ...
    def _analyze_components(self) -> List[ComponentAnalysis]:
        ...
        for component_name, files in component_files.items():
            if len(files) > 1:
                func_count = sum(1 for func in self.ast_index.functions.values() 
                               if func.file_path in files)
                class_count = sum(1 for cls in self.ast_index.classes.values() 
    ...
    def _extract_data_models(self) -> List[Dict[str, Any]]:
        ...
        models = []
        for class_def in self.ast_index.classes.values():
            if (any(keyword in class_def.name.lower() for keyword in ['model', 'entity', 'data']) or
                len(class_def.methods) < 5):
    ...
    def _identify_api_interfaces(self) -> List[Dict[str, Any]]:
        ...
        interfaces = []
        for func_def in self.ast_index.functions.values():
            if (any(keyword in func_def.name.lower() for keyword in ['api', 'endpoint', 'route', 'handler']) or
                any(param in func_def.parameters for param in ['request', 'response', 'req', 'res'])):
    ...
    def _calculate_quality_metrics(self) -> Dict[str, float]:
        ...
        total_files = len(self.ast_index.file_metadata)
        total_functions = len(self.ast_index.functions)
        metrics['avg_functions_per_file'] = total_functions / total_files if total_files > 0 else 0
        
        total_classes = len(self.ast_index.classes)
        total_methods = sum(len(cls.methods) for cls in self.ast_index.classes.values())
        metrics['avg_methods_per_class'] = total_methods / total_classes if total_classes > 0 else 0
        
        documented_functions = sum(1 for func in self.ast_index.functions.values() if func.docstring)
        metrics['documentation_coverage'] = documented_functions / total_functions if total_functions > 0 else 0
    ...
    def _identify_technical_debt(self) -> List[str]:
        ...
        for func in self.ast_index.functions.values():
            if func.docstring and any(keyword in func.docstring.upper() for keyword in ['TODO', 'FIXME', 'HACK']):
                todo_count += 1
    ...
    def _generate_design_recommendations(self) -> List[str]:
        ...
        test_files = sum(1 for path in self.ast_index.file_metadata.keys() 
        ...
        large_files = sum(1 for metadata in self.ast_index.file_metadata.values() 
    ...
    def _identify_layers(self) -> List[str]:
        ...
        for layer_name, keywords in layer_keywords.items():
            if any(keyword in dir_name.lower() for dir_name in directories for keyword in keywords):
    ...
    def _extract_components(self) -> List[str]:
        ...
        components = [d for d in directories if d not in skip_dirs and not d.startswith('.')]
    ...
    def _analyze_data_flow(self) -> Dict[str, Any]:
        ...
        for func_def in self.ast_index.functions.values():
            name_lower = func_def.name.lower()
            if any(keyword in name_lower for keyword in ['read', 'load', 'input', 'fetch']):
                data_flow['input_sources'].append(func_def.name)
            elif any(keyword in name_lower for keyword in ['write', 'save', 'output', 'export']):
                data_flow['output_destinations'].append(func_def.name)
            elif any(keyword in name_lower for keyword in ['process', 'transform', 'convert', 'parse']):
    ...
    def _identify_technology_stack(self) -> List[str]:
        ...
        for import_stmt in self.ast_index.imports:
            ...
            elif any(db in module for db in ['sqlite', 'postgres', 'mysql', 'mongo', 'sqlalchemy']):
    ...
    def _analyze_naming_conventions(self) -> List[CodePattern]:
        ...
        camel_case_funcs = sum(1 for name in function_names if name[0].islower() and any(c.isupper() for c in name))
    ...
    def _detect_structural_patterns(self) -> List[CodePattern]:
        ...
        classes_with_init = sum(1 for cls in self.ast_index.classes.values() 
        ...
        long_functions = sum(1 for func in self.ast_index.functions.values() 
    ...
    def _analyze_import_patterns(self) -> List[CodePattern]:
        ...
        absolute_imports = sum(1 for imp in self.ast_index.imports if not imp.module.startswith('.'))
        relative_imports = sum(1 for imp in self.ast_index.imports if imp.module.startswith('.'))
        
        if absolute_imports > relative_imports * 2:
            patterns.append(CodePattern(
                pattern_type="import",
                description="Prefers absolute imports over relative imports",
                examples=[imp.module for imp in self.ast_index.imports if not imp.module.startswith('.')][:3],
                frequency=absolute_imports,
                confidence=0.8
            ))
        
        stdlib_imports = sum(1 for imp in self.ast_index.imports 
        ...
        if stdlib_imports > 0:
            patterns.append(CodePattern(
                ...
                examples=[imp.module for imp in self.ast_index.imports 
                         if imp.module.split('.')[0] in ['os', 'sys', 'json', 'time', 'datetime']][:3],
    ...
    def _detect_error_handling_patterns(self) -> List[CodePattern]:
        ...
        for func in self.ast_index.functions.values():
            if (any(keyword in func.name.lower() for keyword in ['error', 'exception', 'handle', 'catch']) or
                (func.docstring and any(keyword in func.docstring.lower() for keyword in ['raise', 'except', 'error']))):
    ...
    def _analyze_testing_patterns(self) -> List[CodePattern]:
        ...
        test_files = [path for path in self.ast_index.file_metadata.keys() 
                     if 'test' in Path(path).name.lower()]
        test_functions = [func.name for func in self.ast_index.functions.values() 
    ...
    def _detect_documentation_patterns(self) -> List[CodePattern]:
        ...
        functions_with_docstrings = sum(1 for func in self.ast_index.functions.values() if func.docstring)
        total_functions = len(self.ast_index.functions)
        ...
        classes_with_docstrings = sum(1 for cls in self.ast_index.classes.values() if cls.docstring)
        total_classes = len(self.ast_index.classes)
    ...
    def _extract_code_context(self, file_path: str, line_number: int) -> Tuple[Optional[str], Optional[str]]:
        ...
        for func in self.ast_index.functions.values():
            if (func.file_path == file_path and 
                func.start_line <= line_number <= func.end_line):
                function_name = func.name
                break
        
        for cls in self.ast_index.classes.values():
            if (cls.file_path == file_path and 
                cls.start_line <= line_number <= cls.end_line):
                class_name = cls.name
                break
    ...
    def _find_similar_implementations_for_task(self, task: Task) -> List[ContextualCode]:
    ...
    def _suggest_patterns_for_task(self, task: Task) -> List[CodePattern]:
        ...
        for pattern_list in [patterns.naming_conventions, patterns.structural_patterns, 
                           patterns.import_patterns, patterns.error_handling_patterns]:
            for pattern in pattern_list:
                if any(keyword in pattern.description.lower() for keyword in task_keywords):
