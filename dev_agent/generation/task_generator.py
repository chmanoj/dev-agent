...
from ..models.documents import (
    TaskList, Task, DesignDocument, ComponentSpec, DataModel,
...
class TaskGenerator(ITaskGenerator):
    ...
    def generate_from_design(self, design: DesignDocument) -> TaskList:
        ...
        error_handling_tasks = self._generate_error_handling_tasks(design, task_id_counter)
    ...
    def format_task_list(self, task_list: TaskList) -> str:
        ...
        uncategorized = [t for t in task_list.tasks if self._determine_task_category(t) not in task_order]
        ...
        if task_list.dependencies:
            lines.append(f"**Task Dependencies:** {len(task_list.dependencies)} tasks have dependencies")
    ...
    def _generate_data_model_tasks(self, design: DesignDocument, start_id: int) -> List[Task]:
    ...
    def _generate_interface_tasks(self, design: DesignDocument, start_id: int) -> List[Task]:
        ...
        for interface in design.interfaces:
            task = Task(
                ...
                requirements_refs=self._extract_requirements_from_interface(interface, design),
                subtasks=[],
                status=TaskStatus.NOT_STARTED,
                target_language="python",
    ...
    def _generate_component_tasks(self, design: DesignDocument, start_id: int) -> List[Task]:
        ...
        for component in design.components:
            task = Task(
                ...
                requirements_refs=self._extract_requirements_from_component(component, design),
                subtasks=[],
                status=TaskStatus.NOT_STARTED,
                target_language="python",
    ...
    def _generate_error_handling_tasks(self, design: DesignDocument, start_id: int) -> List[Task]:
    ...
    def _generate_testing_tasks(self, design: DesignDocument, start_id: int) -> List[Task]:
        ...
        if design.testing_strategy.performance_testing and len(design.testing_strategy.performance_testing.strip()) > 0:
            perf_task = Task(
                id=str(task_id),
    ...
    def _generate_integration_tasks(self, design: DesignDocument, start_id: int) -> List[Task]:
    ...
    def _generate_effort_estimates(self, tasks: List[Task]) -> Dict[str, int]:
        ...
        for task in tasks:
            ...
            if task.description:
                ...
                if any(word in desc_lower for word in ['comprehensive', 'complex', 'advanced']):
                    complexity_multiplier += 0.5
                if any(word in desc_lower for word in ['validation', 'security', 'performance']):
    ...
    def _extract_requirements_from_model(self, model: DataModel, design: DesignDocument) -> List[str]:
    ...
    def _extract_requirements_from_interface(self, interface: InterfaceSpec, design: DesignDocument) -> List[str]:
    ...
    def _extract_requirements_from_component(self, component: ComponentSpec, design: DesignDocument) -> List[str]:
    ...
    def _parse_feedback(self, feedback: str) -> Dict[str, Any]:
        ...
        if 'add' in feedback_lower and ('task' in feedback_lower or 'more' in feedback_lower):
    ...
    def _apply_refinements(self, task_list: TaskList, refinements: Dict[str, Any]) -> TaskList:
