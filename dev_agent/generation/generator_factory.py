...
class GeneratorFactory:
    ...
    def __init__(
        self, config: DevAgentConfig, cli_interface: ICLIInterface | None = None
    ):
    ...
    def _should_use_ai_generators(self) -> bool:
        ...
        if (
            not self.config.azure_openai.api_key
            or not self.config.azure_openai.endpoint
        ):
        ...
        try:
            ...
            if service.test_connection():
                ...
                logger.warning(
                    "Azure OpenAI connection failed, falling back to traditional generators"
                )
                return False
...
def create_generators(
    config: DevAgentConfig, cli_interface: ICLIInterface | None = None
