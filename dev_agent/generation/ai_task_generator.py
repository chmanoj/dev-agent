...
class AITaskGenerator(ITaskGenerator):
    ...
    def generate_from_design(self, design: DesignDocument) -> TaskList:
        ...
        try:
            logger.info(
                "Generating implementation tasks from design using Azure OpenAI"
            )
            ...
            logger.info(
                f"Successfully generated {len(task_list.tasks)} implementation tasks"
            )
            return task_list
    ...
    def refine_tasks(self, tasks: TaskList, feedback: str) -> TaskList:
        ...
        try:
            ...
            logger.info(
                f"Successfully refined task list to {len(refined_tasks.tasks)} tasks"
            )
...
Provide the complete refined task list."""
    ...
    def _parse_ai_response_to_tasks(
        ...
        title = (
            design.title + " - Implementation Tasks"
            if design
            else "Implementation Tasks"
        )
        for line in lines:
            if line.startswith("# "):
                title = line[2:].strip()
        ...
        for line in lines:
            ...
            if line.startswith("## ") or (
                ...
                elif line.startswith("**Estimated Hours:**") or line.startswith(
                    "Estimated:"
                ):
                    try:
                        hours_text = line.split(":", 1)[1].strip()
                        current_task["estimated_hours"] = (
                            int("".join(filter(str.isdigit, hours_text))) or 8
                        )
        ...
        for task_data in tasks:
            task = Task(
                ...
                or f"Implement {task_data['title'].lower()}",
