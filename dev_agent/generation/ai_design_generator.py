...
class AIDesignGenerator(IDesignGenerator):
    ...
    def generate_from_specification(
        ...
        try:
            logger.info(
                "Generating design document from specification using Azure OpenAI"
            )
            ...
            design_doc = self._parse_ai_response_to_design(
                response.content, spec, analysis
            )
    ...
    def refine_design(self, design: DesignDocument, feedback: str) -> DesignDocument:
        ...
        try:
            logger.info(
                "Refining design document based on user feedback using Azure OpenAI"
            )
...
Provide the complete refined design document."""
    ...
    def _parse_ai_response_to_design(
        self,
        ai_response: str,
        spec: SpecificationDocument | None = None,
        analysis: DesignAnalysis | None = None,
        base_design: DesignDocument | None = None,
    ) -> DesignDocument:
        ...
        system_architecture = sections.get(
            "system architecture", sections.get("architecture", "")
        )
        component_design = sections.get(
            "component design", sections.get("components", "")
        )
        data_models = sections.get(
            "data models", sections.get("data models and database design", "")
        )
        api_design = sections.get(
            "api design", sections.get("api design and interfaces", "")
        )
        security_considerations = sections.get(
            "security considerations", sections.get("security", "")
        )
        performance_considerations = sections.get(
            "performance and scalability", sections.get("performance", "")
        )
        implementation_guidelines = sections.get(
            "implementation guidelines", sections.get("implementation", "")
        )
