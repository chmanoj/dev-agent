...
class AISpecificationGenerator(ISpecificationGenerator):
    ...
    def __init__(
        ...
        cli_interface: Optional[ICLIInterface] = None,
    ):
    ...
    def generate_from_existing_code(self, analysis: SpecificationAnalysis) -> SpecificationDocument:
        ...
        """
        try:
            logger.info("Generating specification from codebase analysis using Azure OpenAI")
    ...
    def refine_specification(
        ...
        """
        try:
            logger.info("Refining specification based on user feedback using Azure OpenAI")
...
Provide the complete refined specification document."""
    ...
    def _create_analysis_prompt(self, analysis: SpecificationAnalysis) -> str:
        ...
        if analysis.technology_constraints:
            prompt_parts.extend([
                ...
                *[f"- {constraint}" for constraint in analysis.technology_constraints],
                "",
            ])
        ...
        if analysis.requirement_evidence:
            ...
            for evidence in analysis.requirement_evidence[:10]:  # Limit to avoid token overflow
                prompt_parts.extend([
        ...
        prompt_parts.append("Generate a complete, professional specification document based on this analysis.")
    ...
    def _parse_ai_response_to_specification(
        ...
        base_spec: Optional[SpecificationDocument] = None,
    ) -> SpecificationDocument:
        ...
        for i, line in enumerate(lines):
            if any(keyword in line.lower() for keyword in ['introduction', 'overview', 'purpose']):
                intro_start = i + 1
                break
    ...
    def _collect_user_requirements(self) -> List[str]:
        ...
        if not self.cli_interface:
            return ["Implement core functionality", "Provide user interface", "Ensure data persistence"]
        
        requirements = []
        self.cli_interface.display_info("Please provide your requirements (enter empty line to finish):")
        ...
        return requirements or ["Implement core functionality", "Provide user interface", "Ensure data persistence"]