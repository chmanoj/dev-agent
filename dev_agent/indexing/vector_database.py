...
from ..models.indexing import CodeChunk, Embedding, CodeMatch
...
class MockIndex:
    ...
    def search(self, query_vectors, k):
        ...
        indices = np.random.randint(0, len(self.vectors), (len(query_vectors), min(k, len(self.vectors))))
...
class VectorDatabase:
    ...
    def _initialize_model(self) -> None:
        ...
        if SENTENCE_TRANSFORMERS_AVAILABLE:
            try:
                ...
                self.embedding_model = SentenceTransformer(self.embedding_model_name)  # Use mock
        else:
        ...
        try:
            ...
            if self.chunk_mapping_file.exists():
                with open(self.chunk_mapping_file, 'r') as f:
                    ...
                    self.chunk_id_to_index = {k: int(v) for k, v in mapping_data.get('chunk_to_index', {}).items()}
                    self.index_to_chunk_id = {int(k): v for k, v in mapping_data.get('index_to_chunk', {}).items()}
        except Exception as e:
    ...
    def _save_metadata(self) -> None:
        ...
        try:
            ...
            mapping_data = {
                'chunk_to_index': {k: str(v) for k, v in self.chunk_id_to_index.items()},
                'index_to_chunk': {str(k): v for k, v in self.index_to_chunk_id.items()}
            }
    ...
    def store_embedding(self, chunk: CodeChunk, embedding: Optional[np.ndarray] = None) -> str:
    ...
    def query_similar(self, query_text: str, k: int = 10, min_similarity: float = 0.0) -> List[CodeMatch]:
        ...
        similarities, indices = self.index.search(query_embedding.reshape(1, -1), min(k, self.index.ntotal))
    ...
    def query_similar_by_chunk(self, chunk: CodeChunk, k: int = 10, min_similarity: float = 0.0) -> List[CodeMatch]:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        """
        active_chunks = sum(1 for meta in self.metadata_store.values() 
                          if not meta.get('deleted', False) and not meta.get('outdated', False))
        
        return {
            'total_embeddings': self.index.ntotal,
            'active_chunks': active_chunks,
            'deleted_chunks': sum(1 for meta in self.metadata_store.values() if meta.get('deleted', False)),
            'outdated_chunks': sum(1 for meta in self.metadata_store.values() if meta.get('outdated', False)),
