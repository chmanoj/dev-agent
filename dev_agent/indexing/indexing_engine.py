...
from ..models.indexing import CodeChunk, ASTIndex, SymbolInfo
...
class IndexingEngine(IIndexingEngine):
    ...
    def __init__(self, project_path: str, index_path: Optional[str] = None):
        ...
        self.index_path = Path(index_path) if index_path else self.project_path / '.dev_agent' / 'index'
    ...
    def build_index(self) -> IndexResult:
        ...
        try:
            ...
            self._update_progress(len(source_files), len(source_files), "Indexing complete!")
            ...
            print(f"Generated {embeddings_count} embeddings from {len(all_chunks)} chunks")
    ...
    def parse_codebase_ast(self) -> Optional[ASTIndex]:
    ...
    def _parse_file_safe(self, file_path: Path) -> Optional[Dict[str, Any]]:
    ...
    def _merge_ast_data(self, main_ast: ASTIndex, file_ast_data: Dict[str, Any]) -> None:
    ...
    def _parse_single_file(self, file_path: Path, ast_index: ASTIndex) -> None:
        ...
        functions = self.tree_sitter_parser.get_function_definitions(ast, str(file_path))
    ...
    def generate_embeddings(self, code_chunks: List[CodeChunk]) -> List[Any]:
        ...
        for i in range(0, len(code_chunks), batch_size):
            ...
            try:
                ...
                batch_embeddings = self.vector_db._generate_embeddings_batch([chunk.content for chunk in batch])
                all_embeddings.extend(batch_embeddings)
                
                if i % (batch_size * 5) == 0:  # Progress update every 5 batches
                    print(f"Generated embeddings for {min(i + batch_size, len(code_chunks))}/{len(code_chunks)} chunks")
                    
            except Exception as e:
                print(f"Warning: Error generating embeddings for batch {i//batch_size}: {e}")
    ...
    def _discover_source_files(self) -> List[Path]:
        ...
        def should_skip_dir(dir_path: Path) -> bool:
            ...
            return (
                dir_path.name in skip_dirs or
                dir_path.name.startswith('.') and dir_path.name not in {'.github', '.gitlab'}
            )
    ...
    def _save_index_metadata(self, ast_index: ASTIndex, source_files: List[Path], embeddings_count: int) -> None:
    ...
    def _load_existing_index(self) -> bool:
        ...
        try:
            ...
            from ..models.indexing import FunctionDef, ClassDef, Import, SymbolInfo
            ...
            print(f"Loaded existing index with {len(functions)} functions, {len(classes)} classes")
            return True
