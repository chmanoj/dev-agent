...
class EnhancedIndexingEngine(IndexingEngine):
    ...
    def __init__(
        self, project_path: str, config: DevAgentConfig, index_path: str | None = None
    ):
        ...
        logger.info(
            f"Enhanced indexing engine initialized with Azure support: {config.indexing.use_azure_embeddings}"
        )
        ...
        try:
            ...
            if self.config.indexing.use_azure_embeddings:
                if not self.vector_db.test_azure_connection():
                    logger.warning(
                        "Azure OpenAI connection test failed, falling back to local embeddings"
                    )
                    self.config.indexing.use_azure_embeddings = False
                    # Reinitialize vector database with updated config
                    self.vector_db = AzureVectorDatabase(
                        str(self.index_path), self.config
                    )
            ...
            self._update_progress(
                len(source_files), len(source_files), "Enhanced indexing complete!"
            )
            ...
            logger.info(
                f"Enhanced indexing completed in {end_time - start_time:.2f} seconds"
            )
            logger.info(
                f"Indexed {len(source_files)} files, {total_lines} lines of code"
            )
            logger.info(
                f"Generated {embeddings_count} embeddings from {len(all_chunks)} chunks"
            )
            logger.info(
                f"Using Azure embeddings: {self.config.indexing.use_azure_embeddings}"
            )
    ...
    def _generate_enhanced_chunks(
        self, source_files: list[Path], ast_index
    ) -> list[CodeChunk]:
        ...
        for i, file_path in enumerate(source_files):
            try:
                ...
                enhanced_chunks = self._enhance_chunks_with_context(
                    chunks, ast_index, file_path
                )
    ...
    def _enhance_chunks_with_context(
        ...
        for chunk in chunks:
            ...
            for func_key, func_def in ast_index.functions.items():
                if (
                    func_def.file_path == file_str
                    and func_def.start_line <= chunk.start_line <= func_def.end_line
                ):
            ...
            for class_key, class_def in ast_index.classes.items():
                if (
                    class_def.file_path == file_str
                    and class_def.start_line <= chunk.start_line <= class_def.end_line
                ):
    ...
    def query_similar_code(
        ...
        try:
            ...
            logger.info(
                f"Found {len(matches)} similar code matches for query: '{query[:50]}...'"
            )
...
def create_indexing_engine(
    project_path: str, config: DevAgentConfig, index_path: str | None = None
