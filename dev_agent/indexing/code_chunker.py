...
class CodeChunker:
    ...
    def chunk_content(
        self, content: str, file_path: str, language: str = "python"
    ) -> list[CodeChunk]:
    ...
    def chunk_from_ast(self, ast_index: ASTIndex, file_path: str) -> list[CodeChunk]:
        ...
        for func_key, func_def in ast_index.functions.items():
            ...
            chunk_content = self._extract_lines(
                lines, func_def.start_line, func_def.end_line
            )
        ...
        for class_key, class_def in ast_index.classes.items():
            ...
            chunk_content = self._extract_lines(
                lines, class_def.start_line, class_def.end_line
            )
            if chunk_content.strip():
                chunk = CodeChunk(
                    content=chunk_content,
                ...
                if match:
                    ...
                    if len(func_content) <= self.max_chunk_size:
                        ...
                        sub_chunks = self._split_large_function(
                            func_content, file_path, start_line, language
                        )
            ...
            while i < len(lines):
                ...
                if match:
                    ...
                    if len(class_content) <= self.max_chunk_size:
                        ...
                        sub_chunks = self._split_large_class(
                            class_content, file_path, start_line, language
                        )
                        chunks.extend(sub_chunks)
    ...
    def _split_large_function(
        self, func_content: str, file_path: str, start_line: int, language: str
    ) -> list[CodeChunk]:
        ...
        for i, line in enumerate(lines):
            ...
            if should_split or i == len(lines) - 1:
                ...
                if i < len(lines) - 1:
                    overlap_lines = min(
                        self.overlap_size // 20, len(current_chunk)
                    )  # Rough estimate
                    current_chunk = (
                    ...
                    chunk_start_line = (
                        chunk_start_line + len(current_chunk) - overlap_lines
                    )
                else:
                    current_chunk = []
                    current_size = 0
    ...
    def _split_large_class(
        self, class_content: str, file_path: str, start_line: int, language: str
    ) -> list[CodeChunk]:
    ...
    def optimize_chunks(self, chunks: list[CodeChunk]) -> list[CodeChunk]:
        ...
        for chunk in unique_chunks:
            ...
            can_merge = (
                current_chunk.file_path == chunk.file_path
                and current_chunk.language == chunk.language
                and current_chunk.chunk_type == chunk.chunk_type
                and current_chunk.end_line + 1 >= chunk.start_line
                and len(current_chunk.content) + len(chunk.content)
                <= self.max_chunk_size
            )
