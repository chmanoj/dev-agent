...
def configure_azure_openai(
    api_key: Optional[str] = typer.Option(None, "--api-key", help="Azure OpenAI API key"),
    ...
    chat_model: Optional[str] = typer.Option(None, "--chat-model", help="Chat model deployment name"),
    embedding_model: Optional[str] = typer.Option(None, "--embedding-model", help="Embedding model deployment name"),
    interactive: bool = typer.Option(True, "--interactive/--no-interactive", help="Interactive configuration"),
) -> None:
    ...
    if interactive:
        ...
        endpoint = endpoint or _prompt_for_endpoint(current_config.azure_openai.endpoint)
        api_version = api_version or _prompt_for_api_version(current_config.azure_openai.api_version)
        chat_model = chat_model or _prompt_for_chat_model(current_config.azure_openai.chat_model)
        embedding_model = embedding_model or _prompt_for_embedding_model(current_config.azure_openai.embedding_model)
...
def test_azure_connection() -> None:
    ...
    if not config.azure_openai.api_key or not config.azure_openai.endpoint:
        console.print("[red]Error: Azure OpenAI not configured. Run 'dev-agent azure configure' first.[/red]")
        raise typer.Exit(1)
        ...
        with console.status("[bold green]Testing connection..."):
            ...
            try:
                ...
                if response.embeddings:
                    console.print(f"[green]✓ Embeddings test successful (dimension: {len(response.embeddings[0])})[/green]")
                else:
...
def show_azure_status() -> None:
    ...
    table.add_row("API Key", "***" if config.azure_openai.api_key else "Not set", api_key_status)
    table.add_row("Endpoint", config.azure_openai.endpoint or "Not set", endpoint_status)
    ...
    table.add_row("Use Azure Embeddings", str(config.indexing.use_azure_embeddings), 
    ...
    if config.azure_openai.api_key and config.azure_openai.endpoint:
        ...
        console.print("Run [cyan]dev-agent azure configure[/cyan] to set up Azure OpenAI")
...
def list_available_models() -> None:
    ...
    console.print("Use the deployment names from your Azure OpenAI resource, not the base model names.")
...
def show_env_variables() -> None:
    ...
    env_vars = [
        ("AZURE_OPENAI_API_KEY", "Your Azure OpenAI API key", os.getenv("AZURE_OPENAI_API_KEY")),
        ("AZURE_OPENAI_ENDPOINT", "Your Azure OpenAI endpoint URL", os.getenv("AZURE_OPENAI_ENDPOINT")),
        ("AZURE_OPENAI_API_VERSION", "API version (optional)", os.getenv("AZURE_OPENAI_API_VERSION")),
        ("AZURE_OPENAI_CHAT_MODEL", "Chat model deployment name (optional)", os.getenv("AZURE_OPENAI_CHAT_MODEL")),
        ("AZURE_OPENAI_EMBEDDING_MODEL", "Embedding model deployment name (optional)", os.getenv("AZURE_OPENAI_EMBEDDING_MODEL")),
    ]
...
def _prompt_for_api_key(current: Optional[str]) -> str:
    """Prompt for API key."""
    if current:
        if Confirm.ask(f"Keep current API key (***)?"):
            return current
...
def _prompt_for_endpoint(current: Optional[str]) -> str:
