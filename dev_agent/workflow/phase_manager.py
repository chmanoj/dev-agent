...
from ..models.results import (
    PhaseResult, IndexingResult, SpecificationResult, 
    DesignResult, ImplementationResult
)
...
class PhaseManager(IPhaseManager):
    ...
    def __init__(self, cli_interface: ICLIInterface, state_manager: StateManager):
        ...
        self.indexing_engine: Optional[IIndexingEngine] = None
        self.codebase_analyzer: Optional[ICodebaseAnalyzer] = None
        self.specification_workflow: Optional[SpecificationWorkflow] = None
        self.design_workflow: Optional[DesignWorkflow] = None
        self.task_generator: Optional[TaskGenerator] = None
        self.code_generator: Optional[PythonCodeGenerator] = None
        ...
        try:
            ...
            if not self.indexing_engine:
                ...
                def progress_callback(current: int, total: int, message: str):
                    ...
                    self.cli_interface.display_progress(PhaseType.INDEXING, self.current_phase_progress)
                    if message:
            ...
            if not self.indexing_engine.is_index_stale():
                self.cli_interface.display_message("✅ Existing index is up to date, skipping rebuild")
                ...
                if index_result.success:
                    ...
                    result.index_size_mb = index_result.metadata.get('index_size_mb', 0.0)
                    result.languages_detected = index_result.metadata.get('languages_detected', [])
                    ...
                    result.message = f"Indexing failed: {'; '.join(index_result.errors)}"
                    result.errors = index_result.errors
                    
                    self.cli_interface.display_message(f"❌ Indexing failed: {result.message}")
    ...
    def execute_specification_phase(self, context: ProjectContext) -> SpecificationResult:
        ...
        try:
            ...
            if not self.codebase_analyzer:
                if not self.indexing_engine:
                    self.indexing_engine = IndexingEngine(context.project_state.project_path)
    ...
    def execute_design_phase(self, context: ProjectContext) -> DesignResult:
        ...
        try:
            ...
            design = self.design_workflow.execute_design_phase(project_state.specification)
    ...
    def execute_implementation_phase(self, context: ProjectContext) -> ImplementationResult:
        ...
        try:
            ...
            if not project_state or not project_state.design:
                result.status = PhaseStatus.FAILED
                result.message = "No approved design found. Complete design phase first."
                return result
            ...
            self.cli_interface.display_message("Tasks are ready for execution. Use the task execution workflow to implement them.")
    ...
    def validate_phase_completion(self, phase: PhaseType, result: PhaseResult) -> bool:
        ...
        try:
            ...
            if result.errors:
                self.cli_interface.display_message(f"⚠️ Phase completed with warnings: {'; '.join(result.errors)}")
    ...
    def _validate_indexing_completion(self, result: IndexingResult) -> bool:
        ...
        if not result.languages_detected:
            self.cli_interface.display_message("⚠️ Warning: No programming languages detected")
            return False
    ...
    def _validate_specification_completion(self, result: SpecificationResult) -> bool:
        ...
        if result.requirements_count == 0:
            self.cli_interface.display_message("⚠️ Warning: No requirements were generated")
            return False
        
        # Check confidence score
        if result.confidence_score < 0.5:
            self.cli_interface.display_message("⚠️ Warning: Low confidence in specification generation")
        
        # Verify specification was saved
        project_state = self.state_manager.load_project_state()
        if not project_state or not project_state.specification:
            self.cli_interface.display_message("❌ Error: Specification not saved to project state")
            return False
    ...
    def _validate_design_completion(self, result: DesignResult) -> bool:
        ...
        if result.components_count == 0:
            self.cli_interface.display_message("⚠️ Warning: No components were generated")
            return False
        
        # Verify design was saved
        project_state = self.state_manager.load_project_state()
        if not project_state or not project_state.design:
            self.cli_interface.display_message("❌ Error: Design not saved to project state")
            return False
    ...
    def _validate_implementation_completion(self, result: ImplementationResult) -> bool:
        ...
        if not project_state or not project_state.tasks:
            self.cli_interface.display_message("❌ Error: Task list not saved to project state")
            return False
        
        # Check if implementation progress tracking was initialized
        if not project_state.implementation_progress:
            self.cli_interface.display_message("❌ Error: Implementation progress tracking not initialized")
            return False
        
        return True
    
    def retry_phase_execution(self, phase: PhaseType, context: ProjectContext, max_attempts: int = 3) -> PhaseResult:
        ...
        for attempt in range(1, max_attempts + 1):
            self.cli_interface.display_message(f"🔄 Attempt {attempt}/{max_attempts} for {phase.value} phase")
            
            try:
                # Execute phase based on type
                if phase == PhaseType.INDEXING:
                    result = self.execute_indexing_phase(context.project_state.project_path)
                elif phase == PhaseType.SPECIFICATION:
                ...
                if result.status == PhaseStatus.COMPLETED:
                    if attempt > 1:
                        self.cli_interface.display_message(f"✅ {phase.value} phase succeeded on attempt {attempt}")
                    return result
                
                last_result = result
                
                if attempt < max_attempts:
                    self.cli_interface.display_message(f"⚠️ Attempt {attempt} failed: {result.message}")
                    self.cli_interface.display_message(f"Retrying in 2 seconds...")
                    time.sleep(2)
                ...
                if attempt < max_attempts:
                    self.cli_interface.display_message(f"❌ Attempt {attempt} failed with error: {e}")
                    self.cli_interface.display_message(f"Retrying in 2 seconds...")
                    time.sleep(2)
        
        # All attempts failed
        self.cli_interface.display_message(f"❌ All {max_attempts} attempts failed for {phase.value} phase")
    ...
    def get_phase_progress(self, phase: PhaseType) -> float:
        ...
        if phase == self._get_current_executing_phase():
            ...
            project_state = self.state_manager.load_project_state()
            if project_state:
                if phase == PhaseType.INDEXING and project_state.indexing_complete:
                    return 1.0
                elif phase == PhaseType.SPECIFICATION and project_state.specification:
                    return 1.0
                elif phase == PhaseType.DESIGN and project_state.design:
                    return 1.0
                elif phase == PhaseType.IMPLEMENTATION and project_state.tasks:
                    return 1.0
            
            return 0.0
    
    def _get_current_executing_phase(self) -> Optional[PhaseType]:
